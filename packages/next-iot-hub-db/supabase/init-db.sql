...
alter table
  public.authtokens enable row level security;

alter table
  public.categories enable row level security;

alter table
  public.devices enable row level security;
...
create policy "Allow individual delete access" on public.users for delete with check (auth.uid() = id);
...
create policy "Allow individual delete access" on public.devices for delete with check (auth.uid() = id);
...
create policy "Allow individual delete access" on public.projects for delete with check (auth.uid() = id);
...
create policy "Allow individual delete access" on public.authtokens for delete with check (auth.uid() = id);
...
alter table
  public.records drop constraint "records_deviceId_fkey",
add
  constraint "records_deviceId_fkey" foreign key ("deviceId") references devices (id) on delete cascade;

-- devices get deleted by project and by user deletes
alter table
  public.devices drop constraint "devices_projectId_fkey",
add
  constraint "devices_projectId_fkey" foreign key ("projectId") references projects (id) on delete cascade;

alter table
  public.devices drop constraint "devices_userId_fkey",
add
  constraint "devices_userId_fkey" foreign key ("userId") references users (id) on delete cascade;

-- projects get deleted by user delete
alter table
  public.projects drop constraint "projects_userId_fkey",
add
...
alter table
  public.authtokens drop constraint "authtokens_userId_fkey",
add
  constraint "authtokens_userId_fkey" foreign key ("userId") references users (id) on delete cascade;

alter table
  public.authtokens drop constraint "authtokens_projectId_fkey",
add
  constraint "authtokens_projectId_fkey" foreign key ("projectId") references projects (id) on delete cascade;