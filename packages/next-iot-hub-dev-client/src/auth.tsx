...
const Auth = ({ setUserOnClick }: { setUserOnClick: () => void }) => {
  ...
  const forgotPassword = async (e: React.ChangeEvent<any>) => {
      ...
      <div className="flex mt-2">
        {notRegistered ? (
          <span className="block mx-1.5 w-full rounded-md shadow-sm">
            <button
              type="submit"
              onClick={() => handleLogin('SIGNUP').catch(console.error)}
              className={
          ...
          <div className="relative flex justify-center text-sm leading-5">
            <span className="px-2 text-gray-500 bg-white">
              <a
                ...
                className="self-end mt-2 text-sm font-medium text-blue-600 cursor-pointer"
              >
                {notRegistered
                  ? 'You already have an account?'
                  : 'Your are not registered yet?'}
              </a>
            </span>
          </div>
        </div>
      </div>
      {/* social login using github or google */}
      {/* <div className="mt-3">
        <div className="relative">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full mx-1.5 border-t border-gray-300" />
          ...
          <div className="mt-3">
            <span className="block rounded-md shadow-sm">
              <button
                ...
                type="button"
                className="flex justify-center w-3/4 px-4 py-2 mx-auto text-sm font-medium text-white transition duration-150 ease-in-out bg-blue-600 border border-transparent rounded-md hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700"
              >
                GitHub
              </button>
            </span>
